...
from process_pressure import generate_pressuremaps
from process_vorticity import generate_vorticitymaps
from process_platform_pressure import generate_platform_pressuremaps
...
PRESSURE_INPUT         = os.path.join(INPUT_DIR, "pressure")
VORTICITY_INPUT        = os.path.join(INPUT_DIR, "vorticity")
PLATFORM_PRESSURE_INPUT = os.path.join(INPUT_DIR, "platform_pressure")
...
PRESSURE_OUT            = os.path.join(STREAMING, "PressureMaps")
VORTICITY_OUT           = os.path.join(STREAMING, "VorticityMaps")
PLATFORM_PRESSURE_OUT   = os.path.join(STREAMING, "PlatformPressureMaps")
WIND_OUT                = os.path.join(UNITY_PROJECT, "Assets", "Resources", "Streamline")
METADATA_FILE           = os.path.join(STREAMING, "simulation.json")


def ensure_dirs():
    for d in [HEIGHTMAP_OUT, TURBINE_OUT, WIND_OUT, PRESSURE_OUT, VORTICITY_OUT, PLATFORM_PRESSURE_OUT]:
...
def main():
    ...
    generate_pressuremaps(PRESSURE_INPUT, PRESSURE_OUT)
    generate_vorticitymaps(VORTICITY_INPUT, VORTICITY_OUT)
    generate_platform_pressuremaps(PLATFORM_PRESSURE_INPUT, PLATFORM_PRESSURE_OUT)
