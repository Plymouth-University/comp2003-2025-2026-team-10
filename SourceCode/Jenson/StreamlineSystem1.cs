// StreamlineSystem.cs (updated)
// ===============================
// Original wind streamline system — unchanged except for one addition:
// an optional SimulationClock reference. When assigned, particle speed
// is gated by whether the clock is playing, keeping wind and waves in sync.
// If clock is left unassigned, the script behaves exactly as before.
...
public class StreamlineSystem : MonoBehaviour
{
    ...
    [Header("Simulation Sync (optional)")]
    [Tooltip("Assign the SimulationManager here to sync playback with waves.")]
    ...
    // Set to true by SimDataLoader before Start() to skip CSV loading
    [HideInInspector] public bool useBinaryLoader = false;

    void Start()
    {
        // If SimDataLoader is present it calls InjectPaths() directly —
        // skip CSV parsing so we don't load the same data twice.
        if (!useBinaryLoader)
        {
            LoadCSV();
            BuildCombinedPaths();
    ...
    /// <summary>
    /// Called by SimDataLoader to inject pre-parsed binary data directly,
    /// bypassing CSV loading. Paths must already be in Unity coordinate space.
    /// </summary>
    public void InjectPaths(List<List<Vector3>> injectedPaths,
                            List<List<Vector3>> injectedVelocities)
    {
        useBinaryLoader    = true;
        combinedPaths      = injectedPaths;
        combinedVelocities = injectedVelocities;
        Debug.Log($"[StreamlineSystem] Injected {combinedPaths.Count} paths from binary loader.");
    }

    void Update()
    {
        // If clock is assigned, respect its play/pause state
        bool playing = clock == null || clock.isPlaying;
        if (!playing) return;
    ...
    void SpawnParticles()
    {
        foreach (var path in combinedPaths)
        {
            for (int i = 0; i < particlesPerStreamline; i++)
            {
                ...
                particleDistances.Add(Random.Range(0f, path.Count - 1f));
            }
        }
    }
    ...
    void UpdateParticles(float dt)
    {
        ...
        for (int s = 0; s < streamlineCount; s++)
        {
            ...
            for (int i = 0; i < particlesPerStreamline; i++)
            {
                ...
                Vector3 pos = CatmullRom(path[p0], path[p1], path[p2], path[p3], t);
