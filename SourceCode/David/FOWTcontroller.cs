...
public class FOWTController : MonoBehaviour
{
    ...
    [Header("Orientation")]
    public RotationAxis axisOfRotation = RotationAxis.Z; // Default to Z (Forward)
    ...
    void Start()
    {
        if (csvFile) {
            Parse();
            Debug.Log($"<color=green>FOWT:</color> Loaded {snaps.Count} snapshots.");
        }
    }
    ...
    void Update()
    {
        ...
        bladeTransform.Rotate(chosenAxis, rotationStep);
    }
